...
export function MeditationCard({ meditation }: { meditation: Meditation }) {
  const content = (
    <article className="group flex flex-col overflow-hidden rounded-sm border border-border bg-card">
      <div className="relative aspect-video w-full overflow-hidden bg-muted">
        ...
        <span className="absolute left-1/2 top-1/2 flex h-12 w-12 -translate-x-1/2 -translate-y-1/2 items-center justify-center rounded-full bg-background/85 text-primary transition-transform duration-300 group-hover:scale-110">
  ...
  return meditation.href ? <a href={meditation.href} target="_blank" rel="noopener noreferrer">{content}</a> : content
}
